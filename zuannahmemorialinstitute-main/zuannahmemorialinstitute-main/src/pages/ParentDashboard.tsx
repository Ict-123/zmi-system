...
export default function ParentDashboard() {
  ...
  const loading = storeLoading || linksLoading;
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            <p>No students have been linked to your account yet.</p>
            <p className="text-sm mt-2">Please contact the school administrator to link your child's record.</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      {linkedStudents.length > 1 && (
        <div className="flex gap-2 no-print">
          {linkedStudents.map((s) => (
            <Button
              key={s.studentId}
              variant={s.studentId === selectedStudentId ? "default" : "outline"}
              size="sm"
